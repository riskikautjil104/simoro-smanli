...

{
    ...
    public function ujianDetail($id)
    {
        ...
        // Block jika sesi sedang ditandai mencurigakan (belum selesai)
        ...
        // Jika session belum ada, atau sudah di-approve reapply DAN belum selesai, baru boleh aktifkan ujian
        if (!$session) {
            ...
            // Jika session sudah ada, start_time jangan di-reset supaya timer konsisten
        ...
        $now = now();
        // elapsed harus bertambah saat waktu berjalan (jika startTime di masa lalu)
    ...
    public function submitUjian(Request $request, $id)
    {
        ...
        // Permanent block if detected and submitted
        if ($session && $session->is_detected && $session->end_time) {
