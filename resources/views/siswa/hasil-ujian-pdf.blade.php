...
<body>
    ...
    <div class="header">
        ...
        <div class="header-text">
            <div class="school-name">SMA Negeri 5 Kota Gorontalo</div>
            <div class="app-name">SIMORO SMANLI &mdash; Sistem Informasi Ujian Online</div>
            <div class="school-address">Jl. Contoh Alamat No. 123, Gorontalo &bull; Telp. (0435) 000-0000</div>
        </div>
    </div>
    ...
    <table class="detail-table" style="margin-bottom: 24px;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $q->pertanyaan ?? $q->question_text }}</td>
                <td class="option-col">{{ $q->opsi_a ?? '-' }}</td>
                <td class="option-col">{{ $q->opsi_b ?? '-' }}</td>
                <td class="option-col">{{ $q->opsi_c ?? '-' }}</td>
                <td class="option-col">{{ $q->opsi_d ?? '-' }}</td>
                <td class="center">{{ $answers[$q->id]->answer ?? '-' }}</td>
                <td class="center"><b>{{ $q->jawaban_benar ?? $q->answer_key ?? '-' }}</b></td>
                <td class="center">
                    @php
    ...
    <div class="footer">
        <div class="signature-box">
            <div>Gorontalo, {{ now()->format('d F Y') }}</div>
